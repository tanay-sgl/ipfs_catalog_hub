...
add_cid() {
    ...
    # Append the new CID to the content_cids array
    updated_registry=$(cat temp_registry.json | jq --arg cid "$new_cid" '.content_cids += [$cid]')
    ...
    echo "New CID appended to '$name' catalog."
...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <catalog_name> <new_content_cid>"
    exit 1
fi
